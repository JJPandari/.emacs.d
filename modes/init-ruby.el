...
(when (maybe-require-package 'robe)
  ...
  (after-load 'company
    (dolist (hook (mapcar 'derived-mode-hook-name '(ruby-mode inf-ruby-mode html-erb-mode haml-mode)))
      (add-hook hook
                (lambda () (jester/local-push-company-backend 'company-robe))))))
...
(defun jester/ensure-mmm-erb-loaded ()
  (require 'mmm-erb))

(require 'derived)

(defun jester/set-up-mode-for-erb (mode)
  (add-hook (derived-mode-hook-name mode) 'jester/ensure-mmm-erb-loaded)
  (mmm-add-mode-ext-class mode "\\.erb\\'" 'erb))

(let ((html-erb-modes '(html-mode html-erb-mode nxml-mode)))
  (dolist (mode html-erb-modes)
    (jester/set-up-mode-for-erb mode)
    (mmm-add-mode-ext-class mode "\\.r?html\\(\\.erb\\)?\\'" 'html-js)
    (mmm-add-mode-ext-class mode "\\.r?html\\(\\.erb\\)?\\'" 'html-css)))

(mapc 'jester/set-up-mode-for-erb
...
(jester/set-up-mode-for-erb 'yaml-mode)
